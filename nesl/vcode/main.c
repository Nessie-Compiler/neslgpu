...
static inline void ProfileVCodeCALL (prog_entry_t *entry)
{
    double t = GetTime();
    if (++TSP >= TimeStackDepth) ProfileVCodeGrowStack();
    TimeStack[TSP].start = t;
    TimeStack[TSP].child = 0.0;
    TimeStack[TSP].func = entry->func;
}

static inline void ProfileVCodeRETURN ()
{
    FuncTiming_t *curFunc;
    double t = GetTime();
    TimeStackItem_t *top = &(TimeStack[TSP--]);
    double dt = t - top->start;

  // record child time in the timing stack
    TimeStack[TSP].child += dt;

  // record timing information for the function that we are returning from
    curFunc = FuncProfile + top->func;
    curFunc->cumTime += dt;
    dt -= top->child;  // subtract off time spent in the children
    curFunc->time += dt;
    if (curFunc->minTime > dt) curFunc->minTime = dt;
    if (curFunc->maxTime < dt) curFunc->maxTime = dt;
    curFunc->nCalls++;
}
#endif /* VCODE_PROFILE */
...
{
    _fprintf(stderr, "vinterp [-cgt] [-m <num>] [-l <file>] [-<num>] [-d <debug-switch>] file-name\n");
    ...
    _fprintf(stderr, "\t -l <file>\t name of an optional library that implements fused operations\n");
    _fprintf(stderr, "\t -m <num>\t vector stack size in number of doubles (default %d)\n", vstack_size_init);
    _fprintf(stderr, "\t -t\t\t time program execution\n");
#if CUDA
    _fprintf(stderr, "\t -<num>\t use CUDA device <num> (e.g., -0, -1, ...)\n");
#endif
...
char *argv[];
{
...
#else
#if CUDA
    CVL_init(fused_library_name);
#endif
#endif
#endif
#endif

    fused_library_name[0] = '\0';

    while ((c = getopt(argc, argv, "cd:gm:r:tl:0123456789")) != EOF) {
	switch (c) {
	    case 'c':
		check_args = 1;
		break;
		...
		timer = 1;
		break;
	    case 'l':	/* fused library name */
		if (! sscanf(optarg,"%s", fused_library_name))
		    usage();
		break;
	    case 'm':	/* vstack size */
		...
		if (('0' <= c) && (c <= '9')) {
		  // specifying the CUDA device
		    ...
		    cudaError_t sts = cudaSetDevice (dev);
		    if (sts != cudaSuccess) {
			fprintf (stderr, "Error selecting CUDA device %d: %s\n",
			    dev, cudaGetErrorString(sts));
			vinterp_exit(1);
		    }
		}
		else
	...
	cudaError_t sts = cudaGetDevice (&dev);
	if (sts != cudaSuccess) {
	    fprintf (stderr, "Error getting CUDA device: %s\n", cudaGetErrorString(sts));
	    vinterp_exit(1);
	}
	sts = cudaGetDeviceProperties (&prop, dev);
	if (sts != cudaSuccess) {
	    fprintf (stderr, "Error getting CUDA device %d properties: %s\n",
		dev, cudaGetErrorString(sts));
	    vinterp_exit(1);
	}
	fprintf (stderr, "running on CUDA device %d: %s\n", dev, prop.name);
    }
#endif
...
#define START_INSTR_LOGGING 0
#define END_INSTR_LOGGING 0
...
static void main_loop PROTO_((void))
{
	...
	instr_count++;
	if ((START_INSTR_LOGGING != END_INSTR_LOGGING) &&
	    (instr_count > END_INSTR_LOGGING)) {
	    _fprintf(stderr, "Reached END_INSTR: %d\n", instr_count);
	    exit(1);
	}	    
	...
	if (debug_flag) {
	    ...
	    if (value_trace &&
		((START_INSTR_LOGGING == END_INSTR_LOGGING) ||
		 ((instr_count >= START_INSTR_LOGGING) &&
		  (instr_count <= END_INSTR_LOGGING)))) {
		_fprintf(stderr,"\t");
	...
	if ((instruction->vop >= COPY) && (instruction->vop < FUSED)) {		/* non-statements */
	...
	/* Take care of some special cases here */
		...
		case PACK: {
		    ...
		    break;
		default:
            /* All others are FUSED MAKE_SEGDES ops. */
		    if (check_args && se_vb(TOS)->type != Int) {
			_fprintf(stderr, "vinterp: line %d: not applying MAKE_SEGDES to an integer vector.\n",
				instruction->lineno);
			vinterp_exit(1);
        ...
        if (se_vb(stack_args[0])->type == Segdes) {
            length = se_vb(stack_args[0])->seg_len;
        } else {
            length = se_vb(stack_args[0])->len;
        }
        ...
        case Elwise0:
		    (*funct)(dest[0]->vector, 
                     ((dest[0]->type==Segdes)?dest[0]->seg_len:dest[0]->len),
			     SCRATCH);
		    break;
		case Elwise1:
		    (*funct)(dest[0]->vector, 
