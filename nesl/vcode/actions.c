...
#define PROG_MAX_INITIAL	5	/* initial size of program array */
...
{
#ifdef VCODE_PROFILE
    program[next_prog_num].func = NumFuncs-1;
#endif
    ++next_prog_num;
...
/* parse_fused: add a new superoperator to the program.
 */
static int first = 1;

void parse_fused(op, num)
VOPCODE op;
int num;
{
    if (first) {
    ...
    prog->vop = op+num;
    prog->vopdes = vop_fused_table_look(num);
    prog->lineno = yylineno;
    /* This fudge makes it always select the first entry from the
       table of functions */
    prog->type = Int;
    inc_prog_count();
}
