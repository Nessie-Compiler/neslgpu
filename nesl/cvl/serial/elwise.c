...
void rnd_wuz (d, s, len, scratch)
	vec_p d, s, scratch; int len;
{
	register int *dest = (int *) d;
	register int *src = (int *) s;
fprintf(stderr, "rnd_wuz: len = %d, src = %p; src[0] = %d, src[1] = %d\n",
len, src, src[0], src[1]);
	{
		register int _lenS2;
		register int t1,t2,t3,t4;
		switch (len & 0x3) {
			case 3: *dest++ = ((((int)lrand48()))%*src++);
			case 2: *dest++ = ((((int)lrand48()))%*src++);
			case 1: *dest++ = ((((int)lrand48()))%*src++);
			default: ;
		}
		_lenS2 = len >> 2;
		while (_lenS2) {
			dest[0] = ((((int)lrand48()))%src[0]);
			dest[1] = ((((int)lrand48()))%src[1]);
			dest[2] = ((((int)lrand48()))%src[2]);
			dest[3] = ((((int)lrand48()))%src[3]);
			--_lenS2; dest += 4; src += 4;
		}
	}
}
int rnd_wuz_scratch(len) int len; {return 0;}
unsigned int rnd_wuz_inplace() {return 0x001;}
