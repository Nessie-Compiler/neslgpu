...
static int width, height, x0,y0; /* declaring y0 non-static causes linking conflict with math.h Bessel function */
FILE *psstream = NULL;

void Create_Ps(x, y, w, h, filename)
     int x;
     int y;
     int w;
     int h;
     char *filename;
{
  ...
  x0 = x; y0 = y;
  if (psstream == NULL) {
    fprintf(stderr,"Could not open Post Script output file %s\n",filename);
    exit(1);
  }
...
void Create_Window(x, y, width, height, title, displayname, z)
     int x;
     int y;
     int width;
     int height;
...
void keysym()
 {
   ...
   int n1 = get_number();
   int n2 = get_number();
   put_number(XKeycodeToKeysym(display, n1, n2 ));
}
...
void next_event()
{
  ...
  yl = pev->y;
  printf("( %d ) ( %d ) ( %d ) ( %d ) ( %d )\n", 
	 pev->type, xl, yl, pev->state,pev->button);
  fflush(stdout);
}
...
int main()
 {
  ...
  do {   
    ...
    case 5: readdrawtext(); break;
    case 6: clear_window(); break;
